...
build_amd64:
	docker build $(DOCKER_BUILD_ARGS) $(DOCKER_AMD64_ARGS) -t golgoth31/homedynip:$(APP_VERSION) -f ./build/Dockerfile .
	docker push golgoth31/homedynip:$(APP_VERSION)
build_arm64:
	docker build $(DOCKER_BUILD_ARGS) $(DOCKER_ARM64_ARGS) -t golgoth31/homedynip:$(APP_VERSION) -f ./build/Dockerfile .
build_armv7:
	docker build $(DOCKER_BUILD_ARGS) $(DOCKER_ARMV7_ARGS) -t golgoth31/homedynip:$(APP_VERSION) -f ./build/Dockerfile .
lint:
ifndef GOLINTER
...
artifact_linux_amd64:
	GOARCH=amd64 GOPROXY=https://proxy.golang.org CGO_ENABLED=0 GOOS=linux go build -ldflags '${LDFLAGS}' -o homedynip-linux-amd64
artifact_linux_arm64:
	GOARCH=arm64 GOPROXY=https://proxy.golang.org CGO_ENABLED=0 GOOS=linux go build -ldflags '${LDFLAGS}' -o homedynip-linux-arm64
...
artifact_darwin_amd64:
	GOARCH=amd64 GOPROXY=https://proxy.golang.org CGO_ENABLED=0 GOOS=darwin go build -ldflags '${LDFLAGS}' -o homedynip-darwin-amd64
